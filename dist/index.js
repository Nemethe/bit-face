...
const base64_json_1 = __importDefault(require("./base64.json"));
...
const getAvatar = async (seed = "1337", gender = 1) => {
    const canvas = document.createElement('canvas');
    canvas.width = size.width;
    canvas.height = size.height;
    const ctx = canvas.getContext("2d");
    const seededRandom = (0, random_seed_1.create)(seed);
    ...
    const addBase64 = async (base64, color) => {
    ...
    for (let index = 0; index < layersEntries.length; index++) {
        ...
        if (!(key in genderDepend && genderDepend[key] != gender) &&
            !(key in optional && optional[key] < seededRandom.intBetween(1, 100))) {
            let base64 = base64_json_1.default;
            key.split('/').forEach((literal) => {
                base64 = base64[literal];
            });
            if (typeof base64 == 'object') {
                const keys = Object.keys(base64);
